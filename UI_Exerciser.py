...
class LoginTests(unittest.TestCase):
	...
	def pull_element_list_for(self, interest_class):
		...
		self.current_button_list = self.driver.find_elements(by=AppiumBy.CLASS_NAME, value=interest_class)
	...
	def perform_action_on(self, element):
		...
		# TODO: Action chains -> to be revised
	...
	# function to generate application graph

	def print_application_graph(self):

		print("Generating dot file for application graph...", end = "\n\n")
		
		G = nx.MultiDiGraph()
		for hash, screen_info in self.screens.items():

			current_screen_hash_label = str(hash)
			
			items = screen_info.items()
			
			current_screen_next_button_list = list(items)[1][1]
			
			for next_button, next_screen in current_screen_next_button_list:
				
				next_button_label = str(next_button)
				next_screen_label = str(next_screen)

				G.add_edge(current_screen_hash_label, next_screen_label, label=next_button_label)
		
		write_dot(G, 'application_graph.dot')

		print("Please run dot application_graph.dot -T png > output.png to generate .png file of the graph\n")
	...
	@Gooey(body_bg_color='#000000', footer_bg_color='#000000', header_bg_color="#000000")
	def get_program_arguments(self):

		parser = GooeyParser(description="iOS UI Exerciser Script")
